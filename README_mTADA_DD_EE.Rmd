...
$H_1$| $\pi_1$ | $x_{i1} \sim Poisson(2N_1\gamma_1\mu_i)$; $\gamma_1$ \~ $Gamma(\bar{\gamma_1}\beta_1,\beta_1)$  | $x_{i2} \sim Poisson(2N_2\mu_i)$|
$H_2$| $\pi_2$ | $x_{i1} \sim Poisson(2N_1\mu_i)$ | $x_{i2} \sim Poisson(2N_2\gamma_2\mu_i)$; $\gamma_2$ \~ $Gamma(\bar{\gamma_2}\beta_2,\beta_2)$|
...
Data used in the main manuscript are inside the folder [data](data):
  ...
  1) [FullDataSet_DenovoMutations_for_mTADA.txt](data/FullDataSet_DenovoMutations_for_mTADA.txt): all gene-level de novo mutations.

  2) [SingleTrait_Parameters.txt](data/SingleTrait_Parameters.txt): all single-trait parameters. We used $\texttt{extTADA}$ to estimate these parameters.  
...
Only one function $\texttt{mTADA}$ (in the **Run $\texttt{mTADA}$** section) is used to obtain results. However, some additional steps are described here. 

Software versions were used in the example below: $\texttt{R}$ version 3.5.2, $\texttt{locfit}$ version 1.5-9.1, and $\texttt{rstan}$ version 2.18.2.
...
As described above, $\texttt{mTADA}$ needs single-trait parameters: 
  ...
  - the mean and disperson parameters of relative risks: $\bar{\gamma_j}$ and $\beta_j$ (j=$1, 2$);
...
Hoang T. Nguyen, Amanda Dobbyn, Ruth C. Brown, Brien P. Riley, Joseph Buxbaum, Dalila Pinto, Shaun M Purcell, Patrick F Sullivan8, Xin He, Eli A. Stahl. 
